...
class OrderHandler(BaseHandler):
    ...
    def post(self):
        ...
        try:
            sql1="select count(*) counts from ih_order_info where oi_house_id=%s "\
                              "and oi_begin_date<%s and oi_end_date>%s"
            cursor.executemany(sql1,[(house_id,end_date,start_date)])
            ret = cursor.fetchone()
        except Exception as e:
            logging.error(e)
        ...
        try:
            # 保存订单数据ih_order_info，
            sql2 = "insert into ih_order_info(oi_user_id,oi_house_id,oi_begin_date,oi_end_date,oi_days,oi_house_price,oi_amount) values(%s,%s,%s,%s,%s,%s,%s);"
            sql3="update ih_house_info set hi_order_count=hi_order_count+1 where hi_house_id=%s;"
            cursor.executemany(sql2, [(user_id,house_id,start_date,end_date,days,house[0],amount)])
            cursor.executemany(sql3, [(house_id,)])
            db.commit()
        except Exception as e:
            logging.error(e)
...
class AcceptOrderHandler(BaseHandler):
    ...
    def post(self):
        ...
        try:
            # 确保房东只能修改属于自己房子的订单
            sql="update ih_order_info set oi_status=3 where oi_order_id=%s and oi_house_id in "\
                            "(select hi_house_id from ih_house_info where hi_user_id=%s) and oi_status=0"
            cursor.executemany(sql, [(order_id,user_id)])
            db.commit()
        except Exception as e:
            logging.error(e)
...
class OrderCommentHandler(BaseHandler):
    ...
    def post(self):
        ...
        try:
            # 需要确保只能评论自己下的订单
            sql="update ih_order_info set oi_status=4,oi_comment=%s where oi_order_id=%s "\
                            "and oi_status=3 and oi_user_id=%s"
            cursor.executemany(sql, [(comment, order_id, user_id)])
            db.commit()
        except Exception as e:
            logging.error(e)
            return self.write({"errcode":RET.DBERR, "errmsg":"DB error"})
...
class my1(BaseHandler):
    @required_login
    def get(self):
        user_id=self.session.data['session_id']
        role=self.get_argument('role')
